...
def check_system_info() -> Dict[str, Any]:
    """Check basic system information for orchestrator."""
    try:
        import psutil
...
# Removed check_model_formats() as it's only needed for workers
...
def validate_worker_setup() -> bool:
    ...
    if 'error' not in sys_info:
        print_check("System info detected", True)
        print(f"    OS: {sys_info.get('os', 'N/A')} {sys_info.get('os_version', 'N/A')}")
        print(f"    CPU Cores: {sys_info.get('cpu_count', 'N/A')}")
        print(f"    RAM: {sys_info.get('ram_gb', 'N/A')} GB")
    else:
        print_check("System info detected", False, sys_info['error'])
        all_passed = False
    ...
    if 'network_error' not in network:
        print_check("Hostname detected", True, str(network.get('hostname')))
        print_check("Local IP detected", True, str(network.get('ip_address')))
    else:
    ...
    # Model format support section removed - not needed for orchestrator
